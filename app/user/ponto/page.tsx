...
function EspelhoPontoConteudo() {
    ...
    const mapaDadosAgrupados = useMemo(() => {
        ...
        const obterChaveStrict = (funcionarioId: string, dataIsoString: string) => {
            const dataParte = dataIsoString.split('T')[0];
            const [ano, mes, dia] = dataParte.split('-').map(Number);
            return `${funcionarioId}-${ano}-${mes}-${dia}`;
        };
        ...
        pontos.forEach(p => {
            ...
            const chave = obterChaveStrict(p.funcionario_id, p.data_registro);
        ...
        saidasEmergencia.forEach(s => {
            ...
            const chave = obterChaveStrict(s.funcionario_id, s.horario_saida);
            inicializarChave(chave);
    ...
    const obterDadosComExtrasDoDia = (itemDia: DiaCompetencia) => {
        const base = { entrada: '---', saidaAlmoço: '---', voltaAlmoço: '---', saidaFinal: '---', totalPausa: '---', emSaida: '---', emRetorno: '---', emDuracao: '---', justificativa: '', extraDiurnaMinutos: 0, extraNoturnaMinutos: 0, minutosEmergenciaAcumuladoDia: 0, minutosPausaPurosDia: 0, temAtraso: false, textoAjuste: '', descontoDiurno: 0, descontoNoturno: 0, obsPonto: '' };
                        ...
                        <div className="w-full overflow-x-auto print:overflow-visible">
                            <table className="w-full text-left text-xs border-collapse table-auto print:table-fixed min-w-[1050px] print:min-w-0">
                                ...
                                <tr className="border-b border-slate-300 text-slate-800 uppercase font-black text-[9px] tracking-wider bg-slate-100">
                                    ...
                                    <th className="py-2 px-3 text-left w-[200px] print:w-[110px] text-red-700 bg-red-500/5">Motivo Emergência</th>
                                    <th className="py-2 px-2 text-center w-[70px] print:w-[42px] text-orange-600 border-l border-slate-200">Tot Pausa</th>
                                    <th className="py-2 px-2 text-left w-[120px] print:w-[80px] border-l border-slate-100">Obs Batida</th>
                                    <th className="py-2 px-2 text-right min-w-[140px] border-l border-dashed border-slate-300">Ajustes / Eventos</th>
                                </tr>
                                </thead>
                                <tbody>
                                {diasDoCiclo.map((itemDia, idx) => {
                                    ...
                                    return (
                                        <tr key={idx} className={`border-b border-slate-100 text-xs print:text-[9px] print:[color-adjust:exact] [color-adjust:exact] ${j.temAtraso ? 'bg-red-50/70 border-l-4 border-l-red-500' : isAtestado || j.textoAjuste ? 'bg-yellow-50' : itemDia.isFimDeSemana ? 'bg-slate-100/70' : 'hover:bg-slate-50'}`}>
                                            ...
                                            <td className="py-2 px-2 text-left text-[10px] print:text-[8px] text-slate-500 font-medium truncate max-w-[110px] border-l border-slate-100">{j.obsPonto || '---'}</td>
                                            <td className={`py-2 px-2 border-l border-dashed border-slate-200 text-right font-mono font-black text-[9px] uppercase tracking-tight whitespace-nowrap ${isAtestado ? 'text-red-600 font-extrabold' : 'text-amber-700'}`}>
                                                {j.textoAjuste || '---'}
                                            </td>
                                        </tr>
                                    );
